...
CEK, a SECD-based abstract machine. We have shown a complete example on how to
apply this theory on an actual PCF defined function.

This type system was initially defined for a Call-by-name strategy with an
interpretation in a KAM abstract machine, but this approach gave the ability to
formalize d$l$PCF in call-by-value, which is more efficient and more realistic
with actual languages.

Linear dependent types are simply defined on natural integer, but what could be
interesting would be to add them on other types, like floating point
numbers. The next idea is mainly how to add the linear dependent types to
languages that are richer in terms of syntax and semantics, and especially with
more complex types.
